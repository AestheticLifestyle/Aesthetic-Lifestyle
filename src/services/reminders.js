...
export function generateSmartReminders({ dailyCheckin, nutritionLog, weightLog, workoutDone, waterIntake, steps, rules }) {
  ...
  if (hour < 12) {
    if (r.weight_logging && !weightLog) {
      reminders.push({
        type: 'weight_logging',
        titleKey: 'reminderWeighIn',
        messageKey: 'reminderWeighInMsg',
        icon: 'trending-up',
        priority: 'high',
        emoji: '⚖️',
      });
    ...
    if (r.workout_reminder && !workoutDone) {
      reminders.push({
        type: 'workout',
        titleKey: 'workoutToday',
        messageKey: 'workoutTodayDesc',
        icon: 'dumbbell',
        priority: 'medium',
        emoji: '💪',
      });
  ...
  if (hour >= 12 && hour < 18) {
    if (r.meal_logging && !nutritionLog) {
      reminders.push({
        type: 'meal_logging',
        titleKey: 'reminderLogMeals',
        messageKey: 'reminderLogMealsMsg',
        icon: 'utensils',
        priority: 'high',
        emoji: '🍽️',
      });
    ...
    if (r.water_intake && waterIntake < 1.5) {
      reminders.push({
        type: 'water',
        titleKey: 'reminderHydration',
        messageKey: 'reminderHydrationMsg',
        messageParams: { current: waterIntake },
        icon: 'utensils',
        priority: 'medium',
        emoji: '💧',
      });
  ...
  if (hour >= 18) {
    if (r.daily_checkin && !dailyCheckin) {
      reminders.push({
        type: 'daily_checkin',
        titleKey: 'reminderCheckin',
        messageKey: 'reminderCheckinMsg',
        icon: 'clipboard',
        priority: 'high',
        emoji: '📋',
      });
    ...
    if (r.step_target && steps < 8000) {
      ...
      reminders.push({
        type: 'steps',
        titleKey: 'reminderSteps',
        messageKey: 'reminderStepsMsg',
        messageParams: { remaining: remaining.toLocaleString() },
        icon: 'trending-up',
        priority: 'medium',
        emoji: '🚶',
      });
    ...
    if (r.meal_logging && !nutritionLog) {
      reminders.push({
        type: 'meal_logging',
        titleKey: 'reminderLogMeals',
        messageKey: 'reminderLogMealsEvening',
        icon: 'utensils',
        priority: 'high',
        emoji: '🍽️',
      });
